...
/**
 * Listen-only smoke — `withSelfHost` + compiled crm artifact + `/readyz`.
 * `tasks` is injected by `startApplication`. AI is a catalog stub.
 */
test(
	'listens on PGlite selected by Effect Config against the compiled crm artifact',
	...
	async (t) => {
		...
		try {
			await withSelfHost(
				{ bundlePath, tenantId: 'crm-pglite', founder: false },
				async (session) => {
