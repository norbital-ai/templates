...
export default {
	mutate: {
		perRecord: {
			before: {
				description:
					'Books an invoice only against a confirmed purchase order, copies the supplier and currency down from that order, and assigns the next PI document number for the year. Freezes a purchase invoice once it is confirmed or cancelled, requires at least one line to confirm, and requires a cancellation reason to cancel.',
				handler: (context) =>
