...
const before = (input: Record<string, unknown>, context = leaveContext()) =>
	planLeaveBatch(context, [input])[0]!;

test('the entry transform freezes server-measured date charges and ignores caller-supplied quantities', () => {
...
test('entry creation requires complete contract-scoped facts and a supporting reference', () => {
	...
	for (const missing of ['employment_id', 'catalogue_id', 'reference']) {
		assert.throws(
			() => before({ ...input, [missing]: undefined }),
			/employment, leave type and unique reference/
		);
	}
	assert.throws(() => before({ ...input, reference: ' ' }), /unique reference/);
});

test('the entry transform requires the certificate identified by the shared Leave planner', () => {
	const context = leaveContext();
