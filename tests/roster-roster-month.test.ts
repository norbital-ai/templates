 ...
 *
 * `tests/scheduling/roster-month.test.ts` already covers the calendar arithmetic — month
...
test('A1: a local-midnight work_date still attaches the stored row to the calendar cell', () => {
	...
	const facts = buildRosterMonth({
		...
		rosterCodesById: new Map([
			['code-a', DAY_SHIFT],
			['code-rest', { code: 'REST', variant: { kind: 'REST' } }]
		]),
		employmentTerms: [],
		leaveCodeById: new Map(),
		cutoff: null,
	...
	const cell = facts.get(`${EMPLOYMENT}:2026-02-01`);
	assert.equal(cell.workDayId, 'taufik-feb-1');
	assert.equal(cell.designation, 'REST');
	assert.equal(cell.attendanceState, null);
	assert.equal(facts.get(`${EMPLOYMENT}:2026-01-31`), undefined);

	const indexed = indexWorkDaysByPersonDay([
		{ id: 'taufik-feb-1', employment_id: EMPLOYMENT, work_date: stored }
	]);
	assert.equal(indexed.get(`${EMPLOYMENT}:2026-02-01`)?.id, 'taufik-feb-1');
	assert.equal(indexed.get(`${EMPLOYMENT}:2026-01-31`), undefined);
});
