<script lang="ts">
	/**
	 * The ad hoc catalogue's own form: a class of one-off pay — the code, where it settles, who
	 * receives it, what prices and caps it, who raises a request of it (HR, or off-boarding for
	 * the payments the law owes on separation) and the schemes it counts toward.
	 *
	...
	let { record, close }: { record: WorkspaceRow<'adhoc_catalogue'> | null; close: () => void } =
		$props();
			...
			{#snippet limits()}
				<Stack gap="sm">
					...
					<Grid gap="md" minimum="card">
						...
						<Field
							name="raised_by"
							label={t('component.raised_by')}
							description={t('component.raised_by_hint')}
						/>
