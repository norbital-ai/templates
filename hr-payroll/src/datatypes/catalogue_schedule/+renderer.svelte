...
{#if readonly}
	<span class="text-sm">
		{current.every === 'YEAR'
			? t('renderer.catalogue_schedule.summary_year', { month: String(current.month ?? ''), day: String(current.day) })
			: t('renderer.catalogue_schedule.summary_month', { day: String(current.day) })}
		{#if current.from_service_months > 0}
			· {t('renderer.catalogue_schedule.after_service', { months: String(current.from_service_months) })}
		{/if}
		...
		<Grid gap="sm" minimum="compact">
			<label class="flex flex-col gap-1 text-sm">
				...
				<select
					...
					value={current.every}
					onchange={(event) =>
						emit({ every: (event.currentTarget as HTMLSelectElement).value as CatalogueSchedule['every'] })}
				>
			...
			{#if current.every === 'YEAR'}
				<label class="flex flex-col gap-1 text-sm">
					...
					<input
						...
						onchange={(event) => emit({ month: number((event.currentTarget as HTMLInputElement).value, 12) })}
					/>
				</label>
			{/if}
			<label class="flex flex-col gap-1 text-sm">
				...
				<input
					...
					{disabled}
					value={current.day}
					onchange={(event) => emit({ day: number((event.currentTarget as HTMLInputElement).value, 1) })}
				/>
			</label>
			<label class="flex flex-col gap-1 text-sm">
				...
				<input
					...
					onchange={(event) =>
						emit({ from_service_months: number((event.currentTarget as HTMLInputElement).value, 0) })}
				/>
			</label>
			<label class="flex items-center gap-2 text-sm">
				<input
					...
					onchange={(event) => emit({ on_separation: (event.currentTarget as HTMLInputElement).checked })}
				/>
