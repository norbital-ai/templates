 ...
 * of the removed `obligations` collection. `component_catalogue` is the reusable catalogue and policy;
 * the entry owns the amount and carries a real `component_catalogue_id` foreign key, because the
...
export default defineModel(
	...
	{
		description:
			'One employee-specific monetary fact payroll consumes: a claim, a standing allowance, a bonus, an arrears settlement or a manual correction. amount is always a positive magnitude; direction comes from the referenced component policy, and a reversal is computed by the engine against the settled output the correction names.',
