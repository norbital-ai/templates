...
/**
 * ENGINE-OWNED junction — one correction a payslip consumed, captured as an input.
 *
 ...
 *
 * The source carries a global `unique`, which is what the split buys here. A correction is single-use —
...
export default defineModel(
	{
		...
		correction_request_id: uuid().notNull(),
		period: text().notNull()
	},
	{
		description:
			'Engine-owned capture of one correction a payslip consumed. No user policy grants writes on this collection; the payroll engine writes it as part of the run that consumed the request.',
		icon: 'lucide:link',
		indexes: [
			{ columns: ['payslip_id', 'correction_request_id'], unique: true },
			{ columns: ['correction_request_id'], unique: true }
		]
	}
);
