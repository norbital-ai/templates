<script lang="ts">
	/**
	 * A component belongs to one settings version's catalogue. The auto form asked for
...
<svelte:head>
	<meta
		name="bolt:banner"
		content="/__bolt/request/api/template-seed-assets/hr-payroll/record-media/component_catalogue-banner.svg"
	/>
</svelte:head>
	...
	>
		{#snippet children({ Field })}
			<Grid gap="md" minimum="panel">
				...
				<!--
					Which arm an entry raised against this component must declare, and NULL for the
					components the engine feeds. A mutable field must be declared exactly once or the
					form refuses to render at all, so this is not optional decoration.
				-->
				<Field name="entry_kind" label={t('component.entry_kind')} />
