...
- **Claim、Allowance、Payment** 提供已审批的金额记录。奖金、通知期付款、离职补偿及更正使用 Payment 的目录和申请；更正通过原族记录的 `as_adjustment_entry` 表达；类型只向目录行指明的人员提供，目录行要求时必须附收据。**Loan** 管理协议与还款计划；未收回金额由到期金额减已支付捕获推导。
...
| 应用                | 用户在其中做什么                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **实体**            | 选择所有其他 HR 控制器页面使用的法律实体；其「假日」选项卡承载该实体的日历与 Google 来源                                                                                                                                                                                                                                                                                                                             |
...
假日导入使用受管 Google Calendar 连接，需要 `GOOGLE_CALENDAR_API_KEY`，并为各法律实体配置日历标识与时区。假日与目录版本彼此独立；计算规则变更仍通过新的管辖地设置版本处理。租户**不存在 `+seed.ts` 编译器角色**；薪资输入遵循 [`docs/data.md`](docs/data.md) 的种子与对账契约。
