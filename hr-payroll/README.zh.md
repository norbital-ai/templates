...
假日导入使用受管 Google Calendar 连接，需要 `GOOGLE_CALENDAR_API_KEY`，并为各法律实体配置日历标识与时区。年度日历独立于目录版本；计算规则变更仍通过新的管辖地设置版本处理。租户**不存在 `+seed.ts` 编译器角色**；薪资输入遵循 [`docs/data.md`](docs/data.md) 的种子与对账契约。
