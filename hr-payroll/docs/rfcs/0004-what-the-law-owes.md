...
- Changes: what a catalogue row can be the *source* of, and what every expression can *see*. No
...
is not *given* the right inputs, and the rows that would carry the expression do not exist.
...
- **S1 — the year axis and scheduled payments.** `year.*` on every site; `catalogue_schedule`;
  `source` on payment, allowance and claim rows; materialisation of occurrences and separation
  events; `base.entries[].annual_exempt`. Seeds: PH `THIRTEENTH_MONTH_PAY` becomes a SCHEDULE row
  with the ₱90,000 exemption in WTAX; ID `THR` becomes a SCHEDULE row; the KDIT hand-keyed
  `PPH21_BONUS_THR` rows go. Goldens: PH December, PH March leaver, ID Idul Fitri month.
- **S2 — expressions for booleans.** `pay_fraction`, `paid_by`, `base.salary: CONTRACTUAL`,
...
- No new rule vocabulary. Everything above is `when`/`amount` CEL over a richer context, or typed
  data the write validates. A residue that would need a new *operator* is a sign the context is
