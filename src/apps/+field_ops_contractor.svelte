...
>
	<!--
		The table owns the scrolling. The shell grants a definite height, so this Stack fills it and
		the bounded table takes what the filters leave: its rows scroll inside the table while the
		toolbar and pagination bar stay put, and the shell itself never overflows. Wrapping the page
		in a `Scroll` instead let the table grow to its content height and scrolled the pagination
		bar out of reach.
	-->
	<Stack gap="md" fill>
