...
} from './roster-materialization.ts';
...
test('the command returns an existing roster before writes and creates through one atomic graph', () => {
	const source = readFileSync(
		new URL('../../functions/+open_roster_month.ts', import.meta.url),
		'utf8'
	);
	const authoredReads = source.match(/api\.db\.[a-z_]+\.(?:findFirst|findMany)\(/gu) ?? [];
	const authoredWrites = source.match(/api\.db\.[a-z_]+\.mutate\(/gu) ?? [];

	assert.match(source, /OPEN_ROSTER_AUTHORED_QUERY_CEILING = 7/);
	assert.equal(authoredReads.length, 7);
	assert.deepEqual(authoredWrites, ['api.db.rosters.mutate(']);
	assert.equal(source.match(/defineCommandHandler\(/gu)?.length, 1);
	assert.match(source, /work_day_roster: plan\.work_day_roster/);
	assert.doesNotMatch(source, /api\.db\.work_days\.mutate/);
	assert.ok(source.indexOf("state: 'existing'") < source.indexOf('api.db.rosters.mutate'));
});

test('the scheduling action settles the authoritative command and has no local roster enqueue', () => {
	const source = readFileSync(
		new URL('../../apps/hr_controller/+scheduling.svelte', import.meta.url),
		'utf8'
	);
	const start = source.indexOf('function createDraftMonth');
	const end = source.indexOf('/* ─', start);
	assert.ok(start >= 0 && end > start);
	const action = source.slice(start, end);

	// The command is adapted into the effect the action runs, so the success toast and the error
	// banner are both continuations of its settlement — there is no optimistic browser-memory state
	// and no roster row is enqueued locally.
	assert.match(action, /Effect\.tryPromise\(/);
	assert.match(action, /client\.invoke\.open_roster_month/);
	assert.doesNotMatch(action, /client\.db\.rosters\.mutate/);
	assert.match(action, /createDraftError = message/);
});
