...
const FILE = { storage_key: `${UUID}.pdf`, file_name: 'receipt.pdf', file_size: 12, mime_type: 'application/pdf' };
...
test('a column an arm does not use may not be set on it', () => {
	...
	rejects({ terms: 'RECURRING', effective_range: RANGE, occasion: 'CLAIM' }, /cannot carry an occasion/);
	rejects(
		{ terms: 'RECURRING', effective_range: RANGE, instalments: [{ due_date: '2026-04-30', amount: 1 }] },
		/cannot carry an instalment schedule/
	);
	rejects(
		{ terms: 'SCHEDULED', effective_range: RANGE, instalments: [{ due_date: '2026-04-30', amount: 1 }], reverses_obligation_id: UUID },
		/cannot carry a reversed obligation/
	);
	rejects({ terms: 'REVERSAL', reverses_obligation_id: UUID, reason: 'x', effective_range: RANGE }, /cannot carry an effective range/);
	...
	rejects(
		{ terms: 'ONE_OFF', occasion: 'ARREARS', covers_periods: ['2026-01'], reason: 'late', evidence_file: FILE },
...
test('each arm requires what it cannot mean without', () => {
	...
	rejects({ terms: 'ONE_OFF', occasion: 'ARREARS', covers_periods: ['2026-01'] }, /must state a reason/);
	...
	rejects({ terms: 'SCHEDULED', instalments: [{ due_date: '2026-04-30', amount: 1 }] }, /must state the range/);
...
test('an unknown arm or occasion is refused rather than tolerated', () => {
	...
	rejects({ terms: 'ONE_OFF', occasion: 'MANUAL_ADJUSTMENT' }, /not one of ENTERED, CLAIM, ARREARS or ADJUSTMENT/);
