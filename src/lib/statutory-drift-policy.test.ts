...
import statutorySuccessorAutomation from '../access/policies/+statutory_successor_automation.ts';
...
test('the drift worker cannot write law/configuration tables', () => {
	...
	assert.deepEqual(actionsOn(statutoryDriftAutomation, 'employment_statutory_facts'), ['read']);
	assert.deepEqual(actionsOn(statutorySuccessorAutomation, 'employment_statutory_facts'), [
		'create',
		'read',
		'update'
	]);
	for (const action of ['create', 'update']) {
		const grant = statutorySuccessorAutomation.grants.employment_statutory_facts[action];
		assert.equal(typeof grant.approval?.flow, 'function');
		assert.deepEqual(grant.approval?.superceded_by, ['Senior Management']);
	}
	assert.deepEqual(statutorySuccessorAutomation.grants.employment_statutory_facts.create.fields, [
		'employment_id',
	...
	assert.deepEqual(statutorySuccessorAutomation.grants.employment_statutory_facts.update.fields, [
		'effective_range'
	]);
