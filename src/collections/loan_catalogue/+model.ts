import {
	boolean,
	custom,
	defineModel,
	integer,
	sql,
	text,
	uuid
} from '@norbital-ai/bolt/authoring';

export default defineModel(
	{
		/** The jurisdiction settings version this row belongs to, sealed with it. */
		settings_id: uuid().notNull(),
		/** The one label of a pay item. Code and description are the same field; nothing else names it. */
		code: text({ search: true }).notNull(),
		/** The row the law names, as opposed to the entity's own loan schemes. */
		is_statutory: boolean().notNull().default(false),
		/** The component's economic direction: a recovery reduces pay, and the kind says so. */
		policy: custom('component_policy').notNull(),
		/** Read-only projection used for grouping and reporting. */
		nature: text().generatedAlwaysAs(sql`policy ->> 'kind'`),
		/**
		 * How each statutory scheme, by code, charges this recovery. A scheme the map does not name
		 * is undecided and the run refuses at ACCUMULATE naming the component and the scheme.
		 */
		contribution_treatments: custom('contribution_treatments').notNull(),
		/** Where the recovery sits in the reduction order, across every catalogue at once. */
		sequence: integer().notNull(),
		/** One CEL expression over the person context (`payroll_runs/lib/eligibility.ts`); '' is everyone. */
		eligibility: text().notNull().default(''),
		/**
		 * The unit and the settlement route. There is no cap and no evidence: a loan's ceiling is
		 * its principal, which `loans.principal` states and the schedule must sum to, and its
		 * evidence is the agreement.
		 */
		definition: custom('entry_component_definition').notNull()
	},
	{
		description:
			'The loan catalogue of one jurisdiction settings version: the pay lines a loan recovers through, their economic direction, the treatment every statutory scheme gives them, eligibility and settlement route. Sealed with its version; the run cites the version it priced against.',
		recordLabel: ['code'],
		icon: 'lucide:landmark',
		indexes: [{ columns: ['settings_id', 'code'], unique: true }]
	}
);
