...
/**
 * ENGINE-OWNED junction — one arrears settlement a payslip consumed, captured as an input.
 *
 ...
 *
 * The source carries a global `unique`, which is what the split buys here. A arrears settlement is single-use —
...
export default defineModel(
	{
		...
		arrears_request_id: uuid().notNull(),
		period: text().notNull()
	},
	{
		description:
			'Engine-owned capture of one arrears settlement a payslip consumed. No user policy grants writes on this collection; the payroll engine writes it as part of the run that consumed the request.',
		icon: 'lucide:link',
		indexes: [
			{ columns: ['payslip_id', 'arrears_request_id'], unique: true },
			{ columns: ['arrears_request_id'], unique: true }
		]
	}
);
