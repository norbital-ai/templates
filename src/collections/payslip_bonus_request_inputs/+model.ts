...
/**
 * ENGINE-OWNED junction — one bonus a payslip consumed, captured as an input.
 *
 ...
 *
 * The source carries a global `unique`, which is what the split buys here. A bonus is single-use —
...
export default defineModel(
	{
		...
		bonus_request_id: uuid().notNull(),
		period: text().notNull()
	},
	{
		description:
			'Engine-owned capture of one bonus a payslip consumed. No user policy grants writes on this collection; the payroll engine writes it as part of the run that consumed the request.',
		icon: 'lucide:link',
		indexes: [
			{ columns: ['payslip_id', 'bonus_request_id'], unique: true },
			{ columns: ['bonus_request_id'], unique: true }
		]
	}
);
