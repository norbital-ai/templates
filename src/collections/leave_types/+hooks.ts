...
export default {
				...
				handler: ({ input, existing, api }) =>
					Effect.gen(function* () {
						...
						if (row.is_statutory === true && String(row.authority ?? '').trim() === '')
							refuse('A statutory leave type cites the section of law it transcribes.');
						return input;
					})
			},
			after: {
	...
	delete: {
		perRecord: {
			before: {
				description:
					'Refuses deleting a leave type once its jurisdiction settings version is sealed; a type with entitlements on it is history and the restrict edge refuses the delete.',
				handler: ({ existing, api }) =>
					refuseUnlessDraftOnBoth(
						api,
						existing.settings_id,
						undefined,
						`Leave type ${existing.code}`
					)
			}
		}
	}
