...
- **Claim、Allowance、Adhoc** 提供已审批的金额记录。奖金、通知期付款、离职补偿及更正使用 Adhoc 的目录和申请；更正通过原族记录的 `as_adjustment_entry` 表达；类型只向目录行指明的人员提供，目录行要求时必须附收据。**Loan** 管理协议与还款计划；未收回金额由到期金额减已支付回收额推导。
...
- **`statutory_drift`**（每月）：对每个谱系当前生效的版本，通过运行时的页面读取器读取该版本在 `sources.urls` 中命名的官方页面，向模型询问每个法定行的官方现状（方案费率档、法定休假权益、法定薪资项目的计入项），与已封存的行比对；有差异时把该版本克隆为草稿，草稿携带变更后的行与复核清单。「设置」把该草稿标为「法定漂移提议」，HR 复核后由 HR 经理封存。它从不封存、从不触碰已封存版本，每个谱系同时只提议一个草稿；没有来源网址的版本不会被研究。每个无法读取的官方页面都连同原因记录在运行结果和草稿的复核清单上；所有页面都无法读取的谱系不会产生草稿，并在结果中按名称列出。
