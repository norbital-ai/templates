 ...
 *
 * Seven of this workspace's eight representations had never been rendered by any test — including
 * the twelve-hundred-line assignment sheet and the resolution form that closes out a suspicion.
 * This walk renders all of them, and on each surface asks the four questions that fail silently:
 * does it paint without an uncaught error, does anything on it trap the scroll, does its create
 * form lay out real renderers, and was it quick once the workspace was warm.
 */
...
it('every app surface and every representation paints, scrolls and forms cleanly', async () => {
	...
	try {
		...
		const groups = new Set(apps.filter((app) => apps.some((other) => other.startsWith(`${app}/`))));
		...
		// The workspace overview mounts no record-navigation surface, so the detail stack opens over
		// an application route — which is where a tenant opens one.
