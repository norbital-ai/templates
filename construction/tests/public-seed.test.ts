...
test('loadPublicSeed inserts invented construction fixture rows through query', async () => {
	...
	assert.ok(stages.includes('projects'));
	assert.ok(stages.includes('certification_types'));
	assert.ok(stages.includes('workers'));
	...
	assert.deepEqual(
		...
		['projects', 'certification_types', 'workers']
	);
	assert.ok(calls.some((call) => call.parameters.includes('PUB-PRJ-0001')));
	assert.ok(calls.some((call) => call.parameters.includes('PUB-CRT-0001')));
	assert.ok(calls.some((call) => call.parameters.includes('PUB-WRK-0001')));
});
