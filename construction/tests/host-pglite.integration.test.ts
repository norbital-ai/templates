...
/**
 * Listen-only smoke — `withSelfHost` + compiled construction artifact + `/readyz`.
 * `tasks` is injected by `startApplication`. AI is a catalog stub.
 */
test(
	'listens on PGlite selected by Effect Config against the compiled construction artifact',
	...
	async (t) => {
		...
		try {
			await withSelfHost(
				{ bundlePath, tenantId: 'construction-pglite', founder: false },
				async (session) => {
